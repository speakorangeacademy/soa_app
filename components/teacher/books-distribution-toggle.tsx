...
interface BooksDistributionToggleProps {
    studentId: string
    batchId: string
...
export default function BooksDistributionToggle({
    studentId,
    batchId,
    initialStatus,
    initialTimestamp
}: BooksDistributionToggleProps) {
    ...
    const handleToggle = async () => {
        ...
        try {
            const res = await fetch('/api/batch-enrollment/books-distribution', {
                method: 'PATCH',
            ...
            setIsDistributed(result.data.books_distributed)
            setTimestamp(result.data.books_distributed_at)

            if (result.data.books_distributed) {
                setShowSuccess(true)
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            {isDistributed && timestamp && (
                <span style={{ fontSize: '0.7rem', color: 'var(--color-text-muted)', animation: 'fade-in 0.3s' }}>
                    Received: {new Date(timestamp).toLocaleDateString()}
                </span>
            )}
            ...
            <style jsx>{`
                ...
                .books-toggle-track:hover:not(:disabled) {
