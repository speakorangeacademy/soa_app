...
import { CheckCircle2, XCircle, Clock, CreditCard, Calendar, User, BookOpen, Hash, X } from 'lucide-react'
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted flex items-center gap-1 mt-0.5">
                                Transaction ID: <span className="font-mono bg-bg px-1.5 py-0.5 rounded border border-border">{payment.transaction_id}</span>
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <Card className="bg-bg/50 border-none shadow-none">
                        <CardContent className="p-5 flex flex-col gap-1">
                            <span className="text-xs font-semibold text-muted uppercase tracking-wider">Total Amount Paid</span>
                            <div className="flex justify-between items-end">
                                <h1 className="text-4xl font-heading font-bold text-text">
                                    ₹{payment.amount?.toLocaleString()}
                                </h1>
                                <Badge variant={payment.status === 'Approved' ? 'success' : payment.status === 'Rejected' ? 'destructive' : 'warning'} className="px-3 py-1">
                                    {payment.status}
                                </Badge>
                            </div>
                        </CardContent>
                    ...
                    <div className="space-y-4">
                        <h4 className="text-xs font-bold text-muted uppercase tracking-widest border-l-2 border-primary pl-3">Student Information</h4>
                    ...
                    {/* Payment Meta */}
                    <div className="space-y-4">
                        <h4 className="text-xs font-bold text-muted uppercase tracking-widest border-l-2 border-primary pl-3">Transaction Details</h4>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-3 rounded-lg bg-surface border border-border/50">
                                ...
                                <p className="font-medium text-text">{new Date(payment.date).toLocaleDateString(undefined, { dateStyle: 'medium' })}</p>
                            </div>
                            <div className="p-3 rounded-lg bg-surface border border-border/50">
                                <div className="flex items-center gap-2 mb-1 text-muted">
                                    <CreditCard size={14} />
